...
def ask_gpt35(prompt):
    ...
    while True:  # Bucle para reintentar en caso de error
        try:
            # Realiza la solicitud a la API de OpenAI para obtener una respuesta del modelo GPT-3.5-turbo.
            response = openai.ChatCompletion.create(
                ...
                messages=[  # Lista de mensajes que simulan una conversación.
                    {"role": "system", "content": "An undergraduate student"},  # Mensaje del sistema que define el rol del modelo.
                    {"role": "user", "content": prompt}  # Mensaje del usuario que contiene el prompt para el modelo.
                ],
                temperature=1.0,  # Controla la aleatoriedad de la respuesta; un valor alto como 1.0 hace que las respuestas sean más variadas.
            )
...
def log_response_to_excel(run_number, total_amount, gpt_response, full_response, dataframe, error_message=None, filename="dictator_game_log.xlsx"):
    ...
    :param gpt_response: Respuesta procesada del modelo GPT-3.5-turbo.
    ...
    new_row = pd.DataFrame({
        ...
        "GPT Response": [gpt_response],  # Respuesta procesada del modelo GPT-3.5-turbo
...
def dictator_game_gpt35(total_amount, run_number, dataframe, filename):
    """
    Ejecuta el Dictator Game donde el modelo decide cuánto dinero dar al destinatario.
    """
    print("Welcome to the Dictator Game!")
    ...
    full_response = ask_gpt35(prompt)
    
    # Verifica si la respuesta del modelo es válida
    if full_response is None:
        error_message = "Failed to get a valid response from GPT-3.5-turbo."
        print(error_message)
...
df = pd.DataFrame(columns=["Run Number", "Total Amount", "GPT Response", "Full Response", "Error Message"])
...
for i in range(500):
    ...
    recipient_share, df = dictator_game_gpt35(total_amount, i + 1, df, filename)
